...
void GlitchClk_Start(void) 
{
    ...
    GlitchClk_CLKEN |= GlitchClk_CLKEN_MASK;
	GlitchClk_CLKSTBY |= GlitchClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: GlitchClk_Stop
********************************************************************************
*
* Summary:
...
void GlitchClk_Stop(void) 
{
    ...
    GlitchClk_CLKEN &= (uint8)(~GlitchClk_CLKEN_MASK);
	GlitchClk_CLKSTBY &= (uint8)(~GlitchClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: GlitchClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((GlitchClk_CLKEN & GlitchClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = GlitchClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = GlitchClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* GlitchClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(GlitchClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(GlitchClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: GlitchClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: GlitchClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = GlitchClk_GetSourceRegister();
    uint16 oldDivider = GlitchClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = GlitchClk_CLKEN & GlitchClk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((GlitchClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: GlitchClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(GlitchClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: GlitchClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    GlitchClk_MOD_SRC |= modeBitMask & (uint8)GlitchClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: GlitchClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    GlitchClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(GlitchClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: GlitchClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return GlitchClk_MOD_SRC & (uint8)(GlitchClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: GlitchClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = GlitchClk_GetDividerRegister();
    uint8 oldSrc = GlitchClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        GlitchClk_MOD_SRC |= CYCLK_SSS;
        GlitchClk_MOD_SRC =
            (GlitchClk_MOD_SRC & (uint8)(~GlitchClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: GlitchClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return GlitchClk_MOD_SRC & GlitchClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: GlitchClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    GlitchClk_PHASE = clkPhase & GlitchClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: GlitchClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return GlitchClk_PHASE & GlitchClk_PHASE_MASK;
}
